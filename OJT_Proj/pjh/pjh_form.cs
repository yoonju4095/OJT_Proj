...
using OJT_Proj.pjh.Model.SystemCode;
...
{
    public partial class pjh_form : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void Func_Search()
        {
            ...
            /*if (!Check_Search()) return;  //1. 조회 검사

            SC_CorpM corp = new SC_CorpM(); //2. 라인 조회
        ...
        {
            //데이터베이스 저장 기본
            SC_CorpM corp = new SC_CorpM();
            corp.Corp_Cd = Convert.ToString(dr["Corp_Cd"]); //사업장코드 불러와야함 
            if (div != 'D')
            {
                //Delete제외 공통
